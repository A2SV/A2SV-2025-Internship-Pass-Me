...
interface FormComponentProps {
  ...
  handlethepop: (data: any) => void; // opens popup
  popup: boolean;
  flightName: string;
...
const FormComponent: React.FC<FormComponentProps> = ({
  ...
  const [pendingData, setPendingData] = useState<FormDefaultValues | null>(null);
  const openConfirmation = handleSubmit((data) => {
    setPendingData(data);
    handlethepop(data);
  });
  ...
  return (
    <form className="mx-auto w-[85%] mt-10 mb-8">
      ...
      {/* outer “Submit” button now just opens the modal */}
      <div className="flex justify-center items-center mr-60 mt-5">
        <button
          type="button"
          onClick={openConfirmation}
          className="mt-10 w-[141px] h-[40px]
                     rounded-[10px] bg-[#FFFFFF] hover:bg-[#F0F0F0]
                     text-[#3972FF] font-inter font-bold text-[18px]"
        >
      ...
      {popup && (
        <div className="absolute top-1/2 left-1/2
                        w-[90vw] max-w-[550px] h-[300px]
                        bg-black bg-opacity-60 z-50
                        transform -translate-x-1/2 -translate-y-1/2">
          <div className="mt-20 text-white text-center text-[23px]">
            {lanaguage === "am"
          ...
          <div className="flex justify-center gap-4 mt-15">
            <button
              ...
              onClick={() => {
                setPopup(false);
                setPendingData(null);
                handleedit(null);      // back to edit
              }}
              className="bg-[#CCCCCC8C] hover:bg-[#4d4d4d]
                         text-white w-40 h-10 rounded-[12px]
                         font-bold text-[15px]"
            >
              {lanaguage === "am" ? "ወደ ማረም ተመለስ" : "Back to Edit"}
            </button>
            <button
              type="button"
              onClick={confirmSubmission}
              className="bg-[#3972FF] hover:bg-[#5C8BFF]
                         text-white w-40 h-10 rounded-[12px]
                         font-bold text-[20px]"
            >
              {lanaguage === "am" ? "አዎ" : "YES"}
